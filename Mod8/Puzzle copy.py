...
def solve_puzzle(board, source, destination): 
    ...
    while queue:
        ...
        meaning it visits all cells at the current level before moving on to the next level. 
        This ensures that the 1st time the destination cell is encountered during the traversal, 
        it is reached by following the shortest path.
        
        Therefore, the BFS algorithm ensures that the path returned is the shortest path 
        from the source to the destination, provided that the underlying graph (puzzle board) 
        is unweighted."""
